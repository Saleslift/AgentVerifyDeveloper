-- Check if deal_events table already exists
DO $$ 
BEGIN
...
-- Create policies for deal_events if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deal_events' AND policyname = 'Agents can view events for their deals'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deal_events' AND policyname = 'Agents can create events for their deals'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deal_events' AND policyname = 'Agents can update events they created'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deal_events' AND policyname = 'Agents can delete events they created'
  ) THEN
...
DROP TRIGGER IF EXISTS create_meeting_notification_trigger ON deal_events;
