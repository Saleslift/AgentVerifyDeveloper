/*
  # Fix Developer Metadata Handling

  1. Changes
    - Update handle_new_user function to properly extract developer metadata
    - Ensure developer_details contains all fields from signup form
    - Fix existing developer profiles with missing metadata
...
-- Create improved handle_new_user function with better metadata handling
...
BEGIN
  ...
  IF v_role = 'developer' THEN
    -- CRITICAL FIX: Properly build developer_details object with ALL metadata fields
...
-- Extract developer metadata from raw_user_meta_data and update developer_details
UPDATE profiles p
SET 
  role = 'developer',
  developer_details = jsonb_build_object(
    'company_name', COALESCE(u.raw_user_meta_data->>'developerCompanyName', p.full_name),
