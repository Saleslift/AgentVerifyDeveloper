/*
  ...
  1. Changes
    ...
    - Fix the audit log insertion by properly handling user ID
...
SET 
  ...
  updated_at = now()
WHERE 
  id IN (SELECT id FROM auth.users WHERE email = current_user)
  AND role = 'agent';
...
BEGIN
  ...
  SELECT id INTO v_user_id FROM auth.users WHERE email = current_user;
