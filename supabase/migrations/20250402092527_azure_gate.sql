-- Check if deal_activities table already exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'deal_activities') THEN
    ...
    CREATE INDEX idx_deal_activities_agent_id ON deal_activities(agent_id);
    CREATE INDEX idx_deal_activities_timestamp ON deal_activities(timestamp);
  END IF;
END $$;
...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Agents can view activities for their deals" ON deal_activities;
EXCEPTION
...
-- Create or replace functions to automatically log activities
...
DROP TRIGGER IF EXISTS log_chat_activity_trigger ON deal_chats;
DROP TRIGGER IF EXISTS log_file_activity_trigger ON deal_documents;
DROP TRIGGER IF EXISTS log_status_activity_trigger ON deals;
DROP TRIGGER IF EXISTS log_signature_activity_trigger ON deals;
DROP TRIGGER IF EXISTS log_meeting_activity_trigger ON deal_events;
